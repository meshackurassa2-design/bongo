...
const BASE_URL = 'https://kie.ai/suno-api';
...
export const generateMusic = async (
  ...
  const response = await fetch(`${BASE_URL}?model=ai-music-api/generate`, {
    method: 'POST',
    headers: {
...
export const separateVocals = async (audioId: string, audioUrl?: string): Promise<string> => {
  const payload: any = {
    audioId,
  };
  ...
  const response = await fetch(`${BASE_URL}?model=ai-music-api/separate-vocals`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${apiKey}`,
